...
if __name__ == "__main__":
    ...
    try:
        ssh = active_connect(host, int(arm_port), "root", "iraindev10241GB")
        ssh.exec_command(
            "mysqldump -h%s -uroot -piraindb10241GB irain_park new_auth_charge > /home/park/%snew_auth_charge.sql"
            % (host, str(datetime.now())[0:10]))
    except Exception as e:
        print "备份new_auth_charge表失败：%s" % e

    try:
        conn = MySQLdb.connect(host=host, port=3306, user='root', passwd='iraindb10241GB', db='irain_park',
                               charset='utf8')
    except Exception as e:
        print (u'连接数据库异常: %s ，请确认网络连接和数据库状态，如果一直连接不上，请联系研发处理。' % e)
    ...
    try:
        ...
        # new_auth_charge_list = cursor.execute("select * from new_auth_charge;")
        new_auth_charge_list = cursor.fetchall()

        print list(new_auth_charge_list), type(new_auth_charge_list)
        ...
        print "插入new_auth_charge数据报错 %s" % e
